...
Protect external management SSH traffic.



# Behavior

Offerings with text-mode user interaction or CLI _MUST_ 
use SSH version 2, which is Cisco's and the Internet's de facto 
standard protocol for interactive logins. Non text-mode user 
interaction CLI management protocols may also run over SSH. 
An example is NETCONF which can be secured to run over SSH 
version 2 ([RFC6242](https://tools.ietf.org/html/rfc6242)). 

Telnet and SSH version 1 are long obsolete; they _MUST NOT_ be 
enabled by default.
